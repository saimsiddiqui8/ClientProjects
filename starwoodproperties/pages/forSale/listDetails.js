...
export default function ListDetails() {
  ...
  const totalRows = parsedData?.imageUrls ? Math.ceil(parsedData.imageUrls.length / imagesPerRow) : 0; // Calculate the total number of rows
  ...
  const imageRows = Array.from(Array(totalRows), (_, rowIndex) => {
    ...
    return parsedData?.imageUrls ? parsedData.imageUrls.slice(startIndex, endIndex) : [];
  });
    ...
    <div>
      {parsedData && parsedData.names ? (
        <section>
          ...
          <Container>
            <p className='mt-3' id={styles.paragraph}>{parsedData.description}</p>
          </Container>
          <Container className='text-center w-75' id={styles.imgContainer}>
            {imageRows.map((rowImages, rowIndex) => (
              <Row key={rowIndex} className="mt-4">
                {rowImages.map((data, index) => (
                  <Col lg={3} md={3} sm={12} className='mt-3' key={index}>
